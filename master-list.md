...
- **Bugs U + V + W filed S11 day-2 — surfaced building playground-nine (editor IR).**
  ...
  - **Bug V — GENUINE, confirmed post-W (S12). NOT a Bug-W artifact; root cause is lift/reconcile runtime, not codegen.** `class:NAME=expr` on a for-lift element: the `.sel` highlight stays frozen on the create-time winner (first item) and never moves when the dep changes. Re-tested the sidecar against `a91ad5de` under puppeteer: `@sel` advances `0→1→2→0` correctly (W fixed), but the highlight is stuck on `alpha`/id 0 the whole time (only "matches" on click 3 because `@sel` wraps to 0). Static emit shows the per-item effect IS correctly scoped (own `it`, own `_scrml_lift_el_9`, reads `@sel`) — so codegen is fine. Hypothesis sent to scrmlTS: the `innerHTML="" + _scrml_lift(wrapper)` path clones/replaces nodes (or reconcile reuses a node whose create-time effect re-fires on an off-DOM node), so the toggle never reaches the live node. Adopter-common (file lists, menus, tabs, mode badges, our tree/list views). Workaround: render the list as one reactive `${fn()}` string (held in p9). scrmlTS has it queued for post-W diagnosis; our confirmation + diagnostic sent `2026-05-24-0800`.
