...
**Last updated:** 2026-05-30 (S13 cont'd)

**Status:** Exploratory implementation phase, dogfood mode. **Ten scrml-native playgrounds, all green** — p9 13/13, p10 **18/18** against **scrmlTS HEAD `4c9079d2` (v0.7.0)**. p10 (relevance-region navigator) is the first §51-`<engine>` + for-lift-churn playground. **Bug V (class-on-for-lift) RESOLVED + EMPIRICALLY CONFIRMED fixed** on p10's surface (reactive `class:focused`/`style:` follows `@focusId` under nav+reorder+insert+remove churn). **S13 surfaced 5 bugs + 1 engine question (X/Y/Z/AA/AC + AB). scrmlTS landed fixes in S144; 6nz re-tested all against HEAD: X/Y/Z/AA/AC VERIFIED + CLOSED (5/6); AB only PARTIALLY fixed** — write-routing landed (`@mode` advances via dispatcher) but `<onTransition>` effects still never fire (codegen emits empty `__scrml_transitions_mode`, runtime `_scrml_engine_direct_set` fires no hooks). AB reopened with emit+runtime evidence (`2026-05-30-1130`). Four of the six were exit-0 silent miscompiles. Resolutions: X `e50ee9c2`, Z `88071273`, Y/AA `93d8cab4`, AC `c6cd6538`, AB(partial) `5113f3ea`. Bonus: AB probe → scrmlTS ratified **Fork-C1** (`effect=` on the `<engine>` opener as the on-boot effect; impl pending). Deferred to M6 native parser: L (U minor); **T superseded by X (now fixed)**. Editor scaffolding proper still waiting on in-process compiler API.
...
- [ ][ ] Multi-close auto-expansion (editor option, default off) — type `<//>` → expand inline to `</></>`, `<///>` → `</></></>`, Emmet-style; disk + grammar see only canonical `</>` per tag. Optional smart-close (expand to actual tag names via open-tag tracking). scrmlTS dropped Move 7 from the language (S54); the typing-density gain belongs to the editor. (req `2026-05-04-0958-scrmlTS-to-6nz-multi-close-editor-option.md`)
...
- [x] playground-ten — relevance-region navigator. Multiple focused code regions + one mode `<engine>` (NAV/EDIT). Navigate (j/k), reorder (J/K), insert (o), remove (x), edit (Enter→type→Esc). The focus highlight is a reactive `class:focused`/`style:opacity` on a for-lift `<li>` reading global `@focusId` — the Bug-V surface, **confirmed fixed under churn**. `match @mode` mode badge + behaviour gating + dispatcher-routed engine writes. **18/18 smoke against HEAD `4c9079d2`** (incl. live Bug-Z regression guard: a region title `"handleKey(e)"` matches this file's own fn name and must render verbatim). Surfaced bugs X/Y/Z/AA + AB. (`<onTransition>` deliberately left out — AB effect-firing still broken; re-add as a guard once it lands. Follow-ups: promote Tier-0 `${for…lift}` → `<each>`; `^{}` ambient sharing across regions; richer §51 — nested engines / history / onTimeout; `effect=` on `<engine>` opener, Fork-C1.)
...
- **Bugs S + T filed S11 — both surfaced building playground-eight.**
  - Bug S (`return not` mis-emit) — **FIXED + VERIFIED + CLOSED (S126, `3a909c1d`).** The boolean-negation rewrite used `\s+` (crosses newlines) with no keyword exclusion, so a standalone value-completion `not` (`return not`, `@x = not`) ate the next statement's leading token: `return not\nconst x` → `return !const x` (SyntaxError, killed the bundle at load). Fix = two guards at BOTH lowering sites (`expression-parser.ts::preprocessForAcorn` + `codegen/rewrite.ts` arrow-block-body path): (a) `\s+`→`[ \t]+` (horizontal whitespace only); (b) keyword-exclusion lookahead (a JS reserved word is never a negation operand) — standalone `not` falls through to canonical absence. Verified S12 at `3a909c1d`: reverted p8's `return null` workaround (7 sites) back to canonical `return not` + dropped the stale comment; p8 recompiles clean, `node --check` OK, all once-glued sites emit clean `return null;` (zero `return !`). Bare-adjacency repro (`return not` then `const`, no brace) also emits `return null;` un-glued. Closed via `2026-05-24-0814-6nz-to-scrmlTS-bug-s-VERIFIED-closed-workaround-reverted.md`. **This clears the last active fix from our filings — only Bug V remains open (scrmlTS side).** §42.7 asymmetry (literal `null` rejected in value position vs lowered output using `null`) logged by scrmlTS as a separate spec question.
  - Bug T (`//` in string literal) — **deferred to M6 native parser** (sibling of Bug L). BS preprocessing treats `//` inside `"..."` as line-comment start; string truncated AND subsequent `@cell` decls dropped (Bug Q cascade shape). Scope refinement: bites only at module-top `@cell = "...//..."` declaration context, NOT inside function bodies (p8's `lspWsUrl()` returns `"ws://localhost:..."` fine). Workaround: build URLs via `String.fromCharCode(47)` concatenation. **S13 UPDATE: re-filed/broadened as Bug X** — p10 found it bites in function bodies AND `${...}` markup interpolation too (not only module-top decls), for `"..."` and `'...'`, including the everyday `https://`/`http://` URL case → hard `E-CTX-003` with a misleading "logic/program never closed" error. Same root family; Bug X carries the fuller characterization + a URL-severity argument for not waiting on M6.
...
- **Bugs U + V + W filed S11 day-2 — surfaced building playground-nine (editor IR).**
  - **Bug W (CRITICAL) — FIXED + VERIFIED + CLOSED (S126, `a91ad5de`).** Grouping parens were dropped in emit (`(2 + 3) * 4` → `2 + 3 * 4` = 14). scrmlTS root-caused it: acorn parses correctly but discards `ParenthesizedExpression`; the hand-rolled `emitBinary` (codegen/emit-expr.ts) concatenated `left op right` with no precedence guard. Fix = precedence-aware `emitBinary` (precedence table + associativity + ES2020 `??`/`||`/`&&` mixing guard), re-inserting the parens acorn dropped. Verified S12 against `a91ad5de`: emit preserves parens on `(2+3)*4`/`(1+2)*3`/`(10-2)/4`; runtime `(@sel+1)%3` index-wrap advances `0→1→2→0`; p9 recompiles clean (`node --check` OK). Filed `2026-05-24-0641-...-bugs-v-w-from-playground-nine.md` + sidecar; closed via `2026-05-24-0800-6nz-to-scrmlTS-bug-w-VERIFIED-closed-bug-v-GENUINE.md`.
  - **Bug V — RESOLVED upstream (S139 `f8a1f2ff`, shipped v0.6.4/v0.6.7) + EMPIRICALLY CONFIRMED fixed by 6nz (S13).** `class:NAME=expr` on a for-lift element froze on the create-time winner. scrmlTS root cause: a global `_scrml_tracking_paused` flag set across `_scrml_reconcile_list` bled into nested per-item `_scrml_effect`s registered during `createFn(item,i)`, so their `ctx.deps` stayed empty → zero subscribers → never re-fired. Fix: save+null+restore `_scrml_tracking_paused` around the inner `fn()` in `_scrml_effect`/`_scrml_effect_static` (each effect owns its tracking scope). **Class-level** — covers `class:`/`style:`/attr-interp/`textContent`/`bind:value` on any reconciled list item. **6nz S13 confirmation:** built p10's reactive `class:focused`/`style:opacity` on a for-lift `<li>` reading global `@focusId`; the highlight follows focus on j/k, stays on the right region through reorder, and exactly-one-focused holds across insert/remove. 17/17 smoke. The single-string `${fn()}` workaround in p9 can be retired (p10 proves the canonical for-lift+class form); p9 left as-is as a stable regression anchor.
...
- **Bugs X + Y + Z + AA + question AB filed S13 — surfaced building playground-ten. → scrmlTS S144: X/Y/Z/AA CLOSED, AB PARTIAL.** Filed `2026-05-29-1015-6nz-to-scrmlTS-playground-ten-bugs-x-y-z-aa-ab.md` + 5 sidecars; re-test/close-signal `2026-05-30-1130-6nz-to-scrmlTS-s144-5closed-AB-partial.md`. Three are exit-0 silent miscompiles — the shape scrmlTS asked us to hunt. **Resolutions (verified against HEAD `4c9079d2`):** X `e50ee9c2` (line-scoped string-aware `//` gate; URL emitted intact ✅); Z `88071273` (fn-name mangler now string/regex/comment-aware; `"handleKey(e)"` literal preserved ✅); Y `93d8cab4` (hard-rejects `E-MATCH-ARM-SEPARATOR` ✅); AA `93d8cab4` (warns `W-MATCH-VALUE-UNUSED`; `return match`/`fn` canonical ✅). **AB `5113f3ea` — PARTIAL/REOPENED:** the write-routing half landed (program-scope `@mode = .X` now emits `_scrml_engine_direct_set` → dispatcher → `@mode` reactivity + `match` work), but `<onTransition>` effects still never fire — codegen emits `__scrml_transitions_mode = {}` empty (effect bodies not emitted, table not passed to the call) AND the runtime `_scrml_engine_direct_set` has no hook-firing machinery (`grep onTransition` over runtime = 0 firing sites). Needs a second landing (emit effect bodies into the hooks table + fire them in the runtime). Bonus: 6nz's AB probe surfaced a spec gap → scrmlTS ratified **Fork-C1** (`effect=` on the `<engine>` opener as the canonical on-boot/on-enter effect; impl pending).
  - **Bug X (HIGH)** — `//` (incl. `https://`/`http://`) inside a string literal → `E-CTX-003` hard compile failure with a misleading "logic/program never closed" error. String-unaware comment scanner eats to EOL, corrupting brace/context tracking. Bites in logic blocks AND `${...}` markup interp, `"..."` AND `'...'`. Broadens the M6-deferred Bug T (which was scoped to module-top decls only). Workaround: avoid `//` in string literals (build URLs via `String.fromCharCode(47)` concat).
  - **Bug Z (HIGH)** — the identifier-rename pass rewrites a user function-name *substring inside a string literal* (`<label> = "handleKey(e)"` emits `_scrml_reactive_set("label","_scrml_handleKey_3(e)")`). Silent (valid JS, corrupt content). Hit it directly: a p10 region title `"handleKey(e)"` rendered as `_scrml_handleKey_20(e)`. Severe for an editor (text-containing-identifiers is the point). Workaround: region labels avoid this file's own function-name substrings.
  - **Bug Y (MED)** — comma-separated `match` arms compile exit-0 but emit `return X ,;` (invalid JS, `node --check` fails). Canonical newline-separated arms emit clean. Should accept→valid or reject→diagnostic; silent broken emit is the bug. Workaround: newline-separated arms.
  - **Bug AA (MED)** — a bare (non-`return`ed) tail `match` in a plain `function` compiles to a value-discarding IIFE → function returns `undefined` (silent; renders empty). `return match` and `fn name(...) -> T { match }` both emit correct `return (function(){…})()` (this is why p1's `fn modeName` is fine). Workaround: `return match` / promote to `fn`.
  - **Question AB (not a hard bug)** — `<onTransition from=.X to=.Y>` does NOT fire on a bare `@engineVar = .Variant` write from a program-scope function: the write emits a plain `_scrml_reactive_set` that bypasses the engine dispatcher, and the handler table `__scrml_transitions_mode` is emitted empty. `@mode` itself updates reactively (raw `${@mode}` + behaviour gating work); only the `<onTransition>` effect is dead. Asked scrmlTS for the canonical dispatching-transition trigger (distinct from the S140 `effect=`-works finding). p10 leaves `<onTransition>` out pending the answer.
- **Bug AC filed S13 — §36 input-state surface was runtime-dead. → FIXED scrmlTS S144 `c6cd6538` + VERIFIED/CLOSED by 6nz.** Filed `2026-05-29-1130-6nz-to-scrmlTS-bug-ac-input-state-undefined-ref.md` + sidecar. First-contact probe of the "uniquely high-signal" §36 input-state (`<keyboard>`/`<mouse>`) surface. **Bug AC (HIGH)** — every `<#id>` read compiled to a bare undefined identifier `_scrml_input_<id>_` (`_scrml_input_cursor_.x`, `_scrml_input_keys_.pressed(...)`) → `ReferenceError` on first access; exit-0, runtime-dead, whole keyboard/mouse surface incl. the shipped canonical `input-canvas-demo.scrml` sample (compile-only gate — the Bug-51-audit class). **Fix:** reads now resolve through `_scrml_input_state_registry.get("id")` (verified emit `_scrml_input_state_registry.get("cursor").x`; runtime renders `x:0,y:0` with zero pageerrors). +happy-dom gate added upstream. Secondary observation (input-state getters non-reactive) confirmed **by-design** per §36.6 (reads driven by the rAF tick, not a reactive subscription) — accepted, not a gap. No 6nz playground depends on §36 — pure probe.
