...
**Last updated:** 2026-06-20 (S15 — merged the parallel S13/origin fork; S12 re-baseline against v0.7.0 retained)

**Status:** Exploratory implementation phase, dogfood mode. **All ELEVEN scrml-native playgrounds GREEN against scrml v0.7.0 (`80f2c190`, S209)** — p0–p9 re-baselined (p3 CM6 ✓, p5 18/18, p6 7/7, p7 17/17, p8 9/9, p9 13/13; p0/p1/p2/p4 event-fix runtime-probed) + **playground-ten REBUILT (relevance-region navigator + §36 input-state), 19/19 runtime smoke.** The compiler is now `scrml` (renamed from `scrmlTS` at S200; dir `../scrml/`). The S12 re-baseline (p0–p9) found NO compiler bugs (all adopter-migration debt). **The p10 rebuild surfaced 2 NEW compiler bugs (AD, AE) + 1 §36 question (AF)** + recovered/re-verified the lost S13 p10 bug batch (X/Y/Z/AB/AC FIXED, AA open) — see §F. Bugs AD/AE/AF sent to scrml 2026-06-20. Migrations applied: `match` subject typing (E-TYPE-025 → annotate), `=>`→`:>` arm arrows, `const @x`→`const <x>`, `<machine>`→`<engine>` + explicit `initial=`, event-threading arrow-form (S96 revert), bridge LSP path (S200 rename). All S11-era bugs now resolved on scrml's side: **P/Q (fixed), R (retracted), M/N/O (fixed), V (fixed S139), S + W (VERIFIED fixed S12 via direct repro against v0.7.0 — W: `(2+3)*4` preserves parens; S: `return not` → `return null`)**; L/T/U deferred to M6 native parser. Editor scaffolding proper still waiting on in-process compiler API.

> **S13-origin provenance (reconciled at the 2026-06-20 fork merge).** The lost S13 batch was fixed upstream with these compiler SHAs: X `e50ee9c2`, Z `88071273`, Y/AA `93d8cab4`, AC `c6cd6538`, AB `5113f3ea`. The AB probe also ratified **Fork-C1** (`effect=` on the `<engine>` opener as the canonical on-boot effect; impl pending). **Two cross-fork status discrepancies are out for confirmation** (scrml inquiry `2026-06-20-1624`): (a) **AB** — the S13 fork re-tested it only PARTIAL at `4c9079d2` (write-routing landed, `<onTransition>` effects never fired), but our v0.7.0 re-verify saw `<onTransition>` fire (p10 19/19); (b) **AA** — scrml/S13 record it resolved-as-warning (`W-MATCH-VALUE-UNUSED`, `93d8cab4`), our v0.7.0 re-verify flagged it still-open. Both pending scrml's reply.
...
### Bug ledger — S13-recovery + p10 rebuild (verified against v0.7.0, S12)
...
| **AA** | bare tail `match` in a plain `function` silently dropped (value-discard IIFE) | ❌ OPEN — no diagnostic; workaround `return match` / promote to `fn` |
| **AB** | `<onTransition>` didn't fire on bare `@mode=.Variant` write | ✅ FIXED — routes via `_scrml_engine_direct_set` (§51.0.H); live-confirmed in p10 (transitions counter increments) |
| **AC** | `<#cursor>` §36 input-state read → undefined-ref pageerror | ✅ FIXED — reads via `_scrml_input_state_registry` |
| **AD** | *(NEW, p10)* user fn called in an ATTRIBUTE-value interp (`class="x-${fn()}"`) emits the bare un-renamed name → runtime `ReferenceError`. `@cell` refs rewrite fine in the same spot; only user-fn names missed; textContent interp renames correctly. exit-0, node --check OK, runtime-broken. | OPEN — HIGH (Z-family). Workaround: compute via derived cell / avoid fn in attr interp |
| **AE** | *(NEW, p10)* `name=` on an `<engine>` breaks the transition write-guard — guard reads `__scrml_transitions_<EngineName>` (never defined) while the rule table is built under the variable name → `E-ENGINE-001-RT` on every legal transition. | OPEN — HIGH. Workaround: canonical no-`name=` `<engine for=Type>` form |
| **AF** | *(NEW, p10, question)* `${<#input>.field}` in markup renders the initial value once and emits NO reactive effect → never updates on input. Read-in-loop is the only reactive path. | QUESTION — by-design (read in `animationFrame`) vs codegen gap? Real limitation for an editor's live input display |
...
**scrml triage reply (`2026-06-20-1339`, S210 `41422726`, archived):** S13 batch confirmed. **AD FILED HIGH + DISPATCHED** (`g-attr-interp-fn-name-not-renamed` — §47 rewrite misses attr-value template literals; fix in flight). **AE FILED HIGH + DISPATCHED** (`g-engine-name-attr-swallows-var-duplicate` — root: **`name=` is NOT a valid `<engine>` attr**; valid set is `for=`/`initial=`/`var=`/`derived=`; the fix REJECTS `name=` with a "did you mean `var=`?" hint). **AA** tracked for a §18/§19 "match value unused" lint. **AF** is a pending design ruling (render-once may be by-design for mousemove perf; rAF→`@cell` is the supported live-readout pattern). ⚠️ **CARRY: when the AE fix lands, `<engine name=...>` becomes a COMPILE ERROR — p1/p2/p5/p7 all use `<engine name=ModeMachine ...>` and will need migration** (drop `name=`, let the engine own the cell, OR use `var=`). Fine now (fix not landed); migrate before/when it lands.
