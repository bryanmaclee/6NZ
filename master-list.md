...
**Last updated:** 2026-05-29 (S13)

**Status:** Exploratory implementation phase, dogfood mode. **Ten scrml-native playgrounds, all green** — p9 13/13 re-verified against **v0.6.7**, p10 17/17 against **v0.6.7**. p10 (relevance-region navigator) is the first §51-`<engine>` + for-lift-churn playground. **Bug V (class-on-for-lift) RESOLVED upstream (v0.6.4/v0.6.7) and EMPIRICALLY CONFIRMED fixed** on p10's fresh surface (reactive `class:focused`/`style:` follows global `@focusId` under nav + reorder + insert + remove churn; exactly-one-focused invariant holds). Bugs S/W earlier CLOSED. **S13 surfaced 4 new bugs + 1 engine question building p10** (all filed v0.6.7): **X** (HIGH — `//`/URL in string → E-CTX-003), **Z** (HIGH — identifier rewritten inside string literal, silent), **Y** (comma match arms → invalid JS, silent), **AA** (bare tail `match` in plain `function` dropped, silent), **AB** (engine question — `<onTransition>` doesn't fire on bare `@var=.Variant` write). Deferred to M6 native parser: L, T, (U minor). Editor scaffolding proper still waiting on in-process compiler API.
...
- **Bugs X + Y + Z + AA + question AB filed S13 — surfaced building playground-ten (relevance-region navigator), all against v0.6.7.** Filed `2026-05-29-1015-6nz-to-scrmlTS-playground-ten-bugs-x-y-z-aa-ab.md` + 5 sidecars. Three are exit-0 silent miscompiles (compile clean + `node --check`-relevant + runtime-wrong) — the shape scrmlTS asked us to hunt.
