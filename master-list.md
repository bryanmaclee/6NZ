...
- [x][x] `src/playground-eight/` — **works (LSP completion + hover land).** Extends p6's diagnostics-over-WebSocket wiring with `textDocument/completion` (rendered in CM6's `@codemirror/autocomplete` UI) and `textDocument/hover` (rendered as a CM6 tooltip). 8/9 smoke pass: CM6 mounts, LSP reaches ready, initial doc clean, typing `@` returns 57 completion items (first: `lift`) end-to-end through the bridge, typing `<` returns another completion batch, hover request path reachable. 1/9 inconclusive (broken-doc didChange diagnostic re-trigger). Surfaced 2 new compiler bugs (S, T) during construction.
