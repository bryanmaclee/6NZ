...
- **playground-one built**: vim-style mode state machine using scrml's `<machine>` primitive. 5 modes (Insert / Normal / Visual / V-LINE / ToggleHold) with compiler-enforced legal transitions. 6 keyboard transitions wired (Esc, i/a/o, v, V, Ctrl hold/release). Smoke test via puppeteer: 8/8 functional tests pass.
- **New compiler bug found: Bug G** — `fn name(p: T) -> ReturnType { body }` drops the body and emits orphan `- > string {` text at module scope, producing invalid JS. 22-line minimal repro. Filed to scrmlTS at `2026-04-20-1624-6nz-to-scrmlTS-fn-decl-body-dropped.md`. Workaround: use plain `function` instead of `fn` (used in playground-one's `modeName`).
